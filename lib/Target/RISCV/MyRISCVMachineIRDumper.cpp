...
#define RISCV_MIR_DUMPER_PASS_NAME "Dummy RISCV machineIR-Info dumper pass"

namespace {

class RISCVMachineIRDumper : public MachineFunctionPass {
public:
    static char ID;

    RISCVMachineIRDumper() : MachineFunctionPass(ID) { }
    
    bool runOnMachineFunction(MachineFunction &MF) override;
    bool doInitialization(Module &M) override;
    bool doFinalization(Module &M) override;

    StringRef getPassName() const override { return RISCV_MIR_DUMPER_PASS_NAME; }
...
char RISCVMachineIRDumper::ID = 0;

bool RISCVMachineIRDumper::runOnMachineFunction(MachineFunction &MF) {
...
bool RISCVMachineIRDumper::doInitialization(Module &M) {
    // test protobuf
    GOOGLE_PROTOBUF_VERIFY_VERSION;
    return false;
}

bool RISCVMachineIRDumper::doFinalization(Module &M) {
...
INITIALIZE_PASS(RISCVMachineIRDumper, "RISCV-machineIR-dumper",
    RISCV_MIR_DUMPER_PASS_NAME,
    true, // is CFG only?
    true  // is analysis?
)
...
namespace llvm {
// my-RISCV-MIR-dumper
FunctionPass *createRISCVMachineIRDumperPass() { return new RISCVMachineIRDumper(); }
