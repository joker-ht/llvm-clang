...
#define ARM_MACHINEBB_PRINTER_PASS_NAME "Dummy ARM machineBB printer pass"

namespace {

class ARMMachineBBPrinter : public MachineFunctionPass {
public:
    static char ID;

    ARMMachineBBPrinter() : MachineFunctionPass(ID) { }
    
    bool runOnMachineFunction(MachineFunction &MF) override;
    bool doFinalization(Module &M) override;

    StringRef getPassName() const override { return ARM_MACHINEBB_PRINTER_PASS_NAME; }
...
char ARMMachineBBPrinter::ID = 0;

bool ARMMachineBBPrinter::runOnMachineFunction(MachineFunction &MF) {
...
bool ARMMachineBBPrinter::doFinalization(Module &M) {
...
INITIALIZE_PASS(ARMMachineBBPrinter, "ARM-machineBB-printer",
    ARM_MACHINEBB_PRINTER_PASS_NAME,
    true, // is CFG only?
    true  // is analysis?
)
...
FunctionPass *createARMMachineBBPrinterPass() { return new ARMMachineBBPrinter(); }
