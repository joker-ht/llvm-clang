...
#define PPC_MIR_DUMPER_PASS_NAME "Dummy PPC machineIR-Info dumper pass"

namespace {

class PPCMachineIRDumper : public MachineFunctionPass {
public:
    static char ID;

    PPCMachineIRDumper() : MachineFunctionPass(ID) { }
    
    bool runOnMachineFunction(MachineFunction &MF) override;
    bool doInitialization(Module &M) override;
    bool doFinalization(Module &M) override;

    StringRef getPassName() const override { return PPC_MIR_DUMPER_PASS_NAME; }
...
char PPCMachineIRDumper::ID = 0;

bool PPCMachineIRDumper::runOnMachineFunction(MachineFunction &MF) {
...
bool PPCMachineIRDumper::doInitialization(Module &M) {
    // test protobuf
    GOOGLE_PROTOBUF_VERIFY_VERSION;
    return false;
}

bool PPCMachineIRDumper::doFinalization(Module &M) {
...
INITIALIZE_PASS(PPCMachineIRDumper, "PPC-machineIR-dumper",
    PPC_MIR_DUMPER_PASS_NAME,
    true, // is CFG only?
    true  // is analysis?
)
...
namespace llvm {
// my-PPC-MIR-dumper
FunctionPass *createPPCMachineIRDumperPass() { return new PPCMachineIRDumper(); }
