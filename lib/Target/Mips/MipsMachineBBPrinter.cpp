...
#define Mips_MACHINEBB_PRINTER_PASS_NAME "Dummy Mips machineBB printer pass"

namespace {

class MipsMachineBBPrinter : public MachineFunctionPass {
public:
    static char ID;

    MipsMachineBBPrinter() : MachineFunctionPass(ID) { }
    
    bool runOnMachineFunction(MachineFunction &MF) override;
    bool doFinalization(Module &M) override;

    StringRef getPassName() const override { return Mips_MACHINEBB_PRINTER_PASS_NAME; }
...
char MipsMachineBBPrinter::ID = 0;

bool MipsMachineBBPrinter::runOnMachineFunction(MachineFunction &MF) {
...
bool MipsMachineBBPrinter::doFinalization(Module &M) {
...
INITIALIZE_PASS(MipsMachineBBPrinter, "Mips-machineBB-printer",
    Mips_MACHINEBB_PRINTER_PASS_NAME,
    true, // is CFG only?
    true  // is analysis?
)
...
FunctionPass *createMipsMachineBBPrinterPass() { return new MipsMachineBBPrinter(); }
